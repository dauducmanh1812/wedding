  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ name: '', message: '', attending: 'yes', guest: 'none', invitedBy: 'bride' }); // Reset form
      } else {
  ...
  return (
    <div className="App">
      ...
      <section className="section">
        ...
        <h1 className="script-font" style={{fontSize: '3.5rem'}}>{siteData.couple.bride} & {siteData.couple.groom}</h1>
        {/* Family Info */}
      ...
      <section className="section">
        ...
        <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(siteData.event.address)}`} target="_blank" rel="noopener noreferrer">
            CHỈ ĐƯỜNG
        </a>
      </section>
      ...
      <section className="section">
        <h2>Gửi lời chúc</h2>
        <p>Hãy xác nhận sự có mặt của bạn để chúng mình chuẩn bị đón tiếp một cách chu đáo nhất. Trân trọng!</p>
        <form className="rsvp-form" onSubmit={handleSubmit}>
          <input type="text" name="name" placeholder="Tên của bạn" value={formData.name} onChange={handleInputChange} required />
          <textarea name="message" placeholder="Gửi lời nhắn đến cô dâu chú rể" value={formData.message} onChange={handleInputChange} rows="4"></textarea>
          <select name="attending" value={formData.attending} onChange={handleInputChange}>
          ...
          <select name="guest" value={formData.guest} onChange={handleInputChange}>
          ...
          <select name="invitedBy" value={formData.invitedBy} onChange={handleInputChange}>
            <option value="bride">Bạn là khách mời của ai?</option>
            <option value="bride">Cô dâu</option>
            <option value="groom">Chú rể</option>
          </select>
          <button type="submit" className="submit-btn">GỬI LỜI NHẮN</button>
          <button type="button" className="gifting-btn" onClick={() => setShowGiftingModal(true)}>MỪNG CƯỚI</button>
        </form>
      </section>
      ...
      {showThankYouModal && (
        <div className="modal-overlay" onClick={() => setShowThankYouModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close-btn" onClick={() => setShowThankYouModal(false)}>&times;</button>
            <h3 className="script-font">Thank you!</h3>
            <p>Những lời chúc này sẽ là động lực rất lớn giúp chúng mình bước vào một cánh cửa hôn nhân đầy mới mẻ.</p>
      ...
      {showGiftingModal && (
        <div className="modal-overlay" onClick={() => setShowGiftingModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close-btn" onClick={() => setShowGiftingModal(false)}>&times;</button>
            <h3>Mừng cưới</h3>
            <div className="gifting-modal-content">
              <div className="qr-code-container">
                <img src={siteData.gifting.account1.qr} alt="QR Code 1" />
                <p><strong>{siteData.gifting.account1.name}</strong></p>
                <p>{siteData.gifting.account1.bank}</p>
                <p>{siteData.gifting.account1.number}</p>
              </div>
              <div className="qr-code-container">
                <img src={siteData.gifting.account2.qr} alt="QR Code 2" />
                <p><strong>{siteData.gifting.account2.name}</strong></p>
